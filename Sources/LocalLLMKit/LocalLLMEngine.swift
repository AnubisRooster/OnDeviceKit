...
/// Manages a single loaded GGUF model and serves inference requests via
/// [LLM.swift](https://github.com/eastriverlee/LLM.swift) (a Swift wrapper
/// around llama.cpp).
///
/// The engine keeps exactly one model in memory at a time. Loading is done in
/// a background task so the main thread stays responsive. Inference is async
/// and runs inside LLM.swift's `LLMCore` actor (non-main thread), so the
/// engine is safe to call from `@MainActor` contexts without blocking the UI.
@MainActor
public final class LocalLLMEngine: ObservableObject {
    ...
    public func loadModel(id: String, url: URL) async {
    ...
    /// Maps a model ID prefix to the chat template its GGUF was tuned with.
    /// Host apps naming their downloaded models with a `llama`/`phi`/`gemma`
    /// prefix get the right template automatically; anything else falls back
    /// to the ChatML format (Qwen, SmolLM2, and most other instruct-tuned GGUFs).
    public static func template(for modelID: String, systemPrompt: String) -> Template {
    ...
    public static func stopSequence(for modelID: String) -> String {
...
public enum LocalLLMError: LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .notLoaded:
            return "No local model is loaded. Download and select a model first."
        ...
        case .timeout:
            return "Response timed out. The prompt may be too long for this model's context window. Try a smaller model for faster responses."
        }
    }
}
